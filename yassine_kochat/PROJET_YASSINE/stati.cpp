...
int stati::Statistique_partie2()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from CLIENTS where AGE BETWEEN '21' AND '31'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int stati::Statistique_partie3()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from CLIENTS where AGE BETWEEN '32' AND '42'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int stati::Statistique_partie4()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from CLIENTS where AGE BETWEEN '43' AND '53'") ;
    while(requete.next())
    {
            count++ ;
    }
